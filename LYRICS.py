    ...
    def click_open_button(self):
        if self.pressed_open>=0:
            if self.ids.song_box.text !="":
                try:
                    file=open(self.local+self.ids.song_box.text+".txt","r", errors='ignore')
                    self.manager.get_screen("lyrics").ids.song_label.text="\n"*5+(file.read())+"\n"*5
                    self.manager.current="lyrics"
                    self.manager.transition.direction = "left"
                    self.manager.get_screen("add_lyrics").ids.lyrics_ti.text=open(self.local+self.ids.song_box.text+".txt","r", errors='ignore').read()
                    ...
                    try:
                        ...
                        file=open(self.local+".lyrics.txt","r+")

                        if (open_lyrics.name !=False):
                            self.manager.get_screen("lyrics").ids.song_label.text="\n"*5+(file.read())+"\n"*5
                            self.manager.get_screen("add_lyrics").ids.lyrics_ti.text=open(self.local+".lyrics.txt","r+").read()
                            self.manager.get_screen("add_lyrics").ids.path_ti.text=self.ids.song_box.text
                            self.manager.current="lyrics"
                            self.manager.transition.direction = "left"
                        else:
